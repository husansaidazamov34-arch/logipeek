...
export class EmailService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (mailjetApiKey && mailjetSecretKey) {
      try {
        ...
        console.error('❌ Mailjet konfiguratsiya xatosi:', error.message);
  ...
  async sendVerificationCode(email: string, code: string) {
    ...
    try {
      ...
      console.log(`✅ Tasdiqlash kodi yuborildi: ${email} (Mailjet)`);
      return true;
    } catch (error) {
      console.error('❌ Email yuborish xatosi:', error.message || error);
      if (error.response) {
  ...
  async sendPasswordResetCode(email: string, code: string, fullName: string) {
    ...
    try {
      ...
      console.log(`✅ Parol tiklash kodi yuborildi: ${email} (Mailjet)`);
      return true;
    } catch (error) {
      console.error('❌ Email yuborish xatosi:', error.message || error);
      if (error.response) {
